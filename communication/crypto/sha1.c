 ...
 * @brief SHA-1
 *
 * This file contains the implementation of the sha1 algorithm for 'the registry'
 *
 *  Description:
 ...
 *      Date: 19.12.2012
 *      URL: https://www.ietf.org/rfc/rfc3174.txt
 *
 * @file sha1.c
 */
...
int 
sha1(const unsigned char* data, size_t len, uint8_t *res)
{
  if(data == NULL || len == 0 || res == NULL)
    return ERROR_INVALID_ARGUMENTS;

  SHA1Context context;
  if(SHA1Reset(&context) != shaSuccess ||
     SHA1Input(&context, data, len) != shaSuccess ||
     SHA1Result(&context, res) != shaSuccess)
